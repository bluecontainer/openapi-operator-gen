...
var (
	...
	commit           = "unknown"
	date             = "unknown"
	generatorVersion = "v0.0.10-12-gc00c42a-dirty"
...
func main() {
	...
	// Static URL mode
	var baseURL string
	var baseURLsFlag string // comma-separated list of base URLs
	...
	flag.StringVar(&baseURLsFlag, "base-urls", "", "Comma-separated list of base URLs for fan-out mode (writes to all, reads use first success)")
	...
	flag.StringVar(&basePath, "base-path", "", "Base path prefix for REST API on pods (e.g. /api/v3)")
	...
	flag.StringVar(&watchLabels, "watch-labels", "", "Only watch CRs matching these labels (format: key1=value1,key2=value2)")
	flag.StringVar(&watchNamespaces, "watch-namespaces", "", "Only watch CRs in these namespaces (format: ns1,ns2,ns3). Empty means all namespaces.")
	flag.BoolVar(&namespaceScoped, "namespace-scoped", false, "Only watch CRs in the operator's own namespace (auto-detected from service account)")
	...
	if baseURLsFlag == "" {
		baseURLsFlag = os.Getenv("REST_API_BASE_URLS")
	}

	// Parse baseURLs from comma-separated flag/env var
	var baseURLs []string
	if baseURLsFlag != "" {
		for _, u := range strings.Split(baseURLsFlag, ",") {
	...
	if podName == "" {
		podName = os.Getenv("TARGET_POD_NAME")
	}
	if helmRelease == "" {
	...
	if !namespaceScoped && os.Getenv("NAMESPACE_SCOPED") == "true" {
		namespaceScoped = true
	}

	// Parse watch namespaces into a list
	var namespaceList []string

	// Handle namespace-scoped mode: auto-detect operator's namespace
	if namespaceScoped {
		operatorNS, err := os.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/namespace")
		if err != nil {
			setupLog.Error(err, "failed to detect operator namespace for namespace-scoped mode")
			os.Exit(1)
		}
		namespaceList = []string{strings.TrimSpace(string(operatorNS))}
		setupLog.Info("Running in namespace-scoped mode", "namespace", namespaceList[0])
		...
		if len(namespaceList) > 0 {
			setupLog.Info("Filtering CRs by namespaces", "namespaces", namespaceList)
		}
	}

	// Parse watch labels into a label selector
	var labelSelector labels.Selector
	if watchLabels != "" {
		labelMap := make(map[string]string)
		for _, pair := range strings.Split(watchLabels, ",") {
			...
			if len(parts) == 2 {
				...
				setupLog.Error(fmt.Errorf("invalid label format: %s", pair), "failed to parse watch-labels")
				os.Exit(1)
			}
		}
		if len(labelMap) > 0 {
			labelSelector = labels.SelectorFromSet(labelMap)
			setupLog.Info("Filtering CRs by labels", "selector", labelSelector.String())
		}
	}

	// Check if global endpoint configuration is provided
	useWorkloadDiscovery := stsName != "" || deployName != "" || podName != "" || helmRelease != ""
	hasGlobalConfig := useWorkloadDiscovery || baseURL != "" || len(baseURLs) > 0

	if !hasGlobalConfig {
		setupLog.Info("No global endpoint configuration provided - CRs must specify target.baseURL, target.baseURLs, target.pod, target.helmRelease, target.statefulSet, or target.deployment")
	}
	...
	// Configure cache filtering based on namespaces and/or labels
	if len(namespaceList) > 0 || labelSelector != nil {
		...
		// Configure label filtering per object type
		if labelSelector != nil {
			cacheOpts.ByObject = map[client.Object]cache.ByObject{
				...
				&v1alpha1.PetstoreAggregate{}: {
					Label: labelSelector,
				},
				&v1alpha1.PetstoreBundle{}: {
					Label: labelSelector,
				},
			}
		}
	...
	if useWorkloadDiscovery && podName == "" {
		...
		setupLog.Info("Using static base URLs (fan-out mode)", "urls", baseURLs)
	} else if baseURL != "" {
