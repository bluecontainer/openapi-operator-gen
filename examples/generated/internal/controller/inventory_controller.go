...
const (
	inventoryFinalizer    = "petstore.example.com/finalizer"
	inventoryRequeueAfter = time.Second * 30
)
...
// +kubebuilder:rbac:groups=petstore.example.com,resources=inventories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=petstore.example.com,resources=inventories/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=petstore.example.com,resources=inventories/finalizers,verbs=update
...
func (r *InventoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	instance := &v1alpha1.Inventory{}
	err := r.Get(ctx, req.NamespacedName, instance)
	if err != nil {
		if errors.IsNotFound(err) {
			logger.Info("Inventory resource not found. Ignoring since object must be deleted")
	...
	if instance.GetDeletionTimestamp() != nil {
		if controllerutil.ContainsFinalizer(instance, inventoryFinalizer) {
			...
			// Remove finalizer
			controllerutil.RemoveFinalizer(instance, inventoryFinalizer)
	...
	if !isReadOnly && !controllerutil.ContainsFinalizer(instance, inventoryFinalizer) {
		controllerutil.AddFinalizer(instance, inventoryFinalizer)
		if err := r.Update(ctx, instance); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	if isReadOnly {
		if instance.Spec.ExternalIDRef == "" {
			...
			return ctrl.Result{RequeueAfter: inventoryRequeueAfter}, fmt.Errorf("readOnly mode requires externalIDRef")
		}
		if err := r.observeResource(ctx, instance); err != nil {
			r.updateStatus(ctx, instance, "Failed", err.Error())
			return ctrl.Result{RequeueAfter: inventoryRequeueAfter}, err
		}
		return ctrl.Result{RequeueAfter: inventoryRequeueAfter}, nil
	}
...
func (r *InventoryReconciler) getBaseURL(ctx context.Context) (string, error) {
...
func (r *InventoryReconciler) getBaseURLByOrdinal(ctx context.Context, ordinal *int32) (string, error) {
...
func (r *InventoryReconciler) getExternalID(instance *v1alpha1.Inventory) string {
	if instance.Spec.ExternalIDRef != "" {
...
func (r *InventoryReconciler) getResource(ctx context.Context, baseURL string, externalID string) (map[string]interface{}, []byte, error) {
	logger := log.FromContext(ctx)

	url := baseURL + "/store/inventory/" + externalID
...
func (r *InventoryReconciler) compareSpecWithResponse(instance *v1alpha1.Inventory, apiResponse map[string]interface{}) bool {
...
func (r *InventoryReconciler) observeResource(ctx context.Context, instance *v1alpha1.Inventory) error {
	logger := log.FromContext(ctx)
	...
	if r.EndpointResolver != nil && r.EndpointResolver.IsAllHealthyStrategy() {
		...
		if len(baseURLs) > 1 {
			...
			responses := make(map[string]v1alpha1.InventoryEndpointResponse)
			successCount := 0
			var firstSuccessBody []byte

			for _, baseURL := range baseURLs {
				endpointResp := v1alpha1.InventoryEndpointResponse{
					LastUpdated: &now,
				}
			...
			if firstSuccessBody != nil {
				instance.Status.Response = &v1alpha1.InventoryEndpointResponse{
					Success:     true,
					StatusCode:  200,
...
func (r *InventoryReconciler) resolveBaseURL(ctx context.Context, instance *v1alpha1.Inventory) (string, error) {
	if r.EndpointResolver != nil {
...
func (r *InventoryReconciler) syncToEndpoint(ctx context.Context, instance *v1alpha1.Inventory, baseURL string) error {
	logger := log.FromContext(ctx)
...
func (r *InventoryReconciler) createResource(ctx context.Context, instance *v1alpha1.Inventory, baseURL string) error {
	logger := log.FromContext(ctx)
...
func (r *InventoryReconciler) updateResource(ctx context.Context, instance *v1alpha1.Inventory, baseURL string, externalID string) error {
	logger := log.FromContext(ctx)
...
func (r *InventoryReconciler) marshalSpecForAPI(instance *v1alpha1.Inventory) ([]byte, error) {
	specData, err := json.Marshal(instance.Spec)
	if err != nil {
		return nil, err
	}
...
func (r *InventoryReconciler) syncResource(ctx context.Context, instance *v1alpha1.Inventory) error {
	logger := log.FromContext(ctx)
...
func (r *InventoryReconciler) deleteFromEndpoint(ctx context.Context, instance *v1alpha1.Inventory, baseURL string) error {
	logger := log.FromContext(ctx)

	url := baseURL + "/store/inventory/" + instance.Status.ExternalID
...
func (r *InventoryReconciler) finalizeResource(ctx context.Context, instance *v1alpha1.Inventory) error {
	logger := log.FromContext(ctx)
...
func (r *InventoryReconciler) updateStatus(ctx context.Context, instance *v1alpha1.Inventory, state, message string) {
	logger := log.FromContext(ctx)
