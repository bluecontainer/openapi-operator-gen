...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	// Create the CR
	isStructuredOutput := outputFormat == "json" || outputFormat == "yaml"
	...
	if !createNoWait {
		return waitForCreateResult(ctx, plural, name, isStructuredOutput)
	}

	// No-wait: output result immediately
	if isStructuredOutput {
...
func isCreateCommonFlag(name string) bool {
	...
	commonFlags := map[string]bool{
		...
		"timeout": true, "output": true, "compact": true,
		"from-file": true, "dry-run": true,
	}
...
func waitForCreateResult(ctx context.Context, plural, name string, isStructuredOutput bool) error {
	if !isStructuredOutput {
...
func printCreateResult(obj *unstructured.Unstructured, elapsed time.Duration) error {
	...
	// Structured output: clean envelope with CR metadata + response data
	if outputFormat == "json" || outputFormat == "yaml" {
		...
		if elapsed > 0 {
			result["syncTime"] = elapsed.Round(time.Millisecond).String()
		}
		if outputFormat == "json" {
			return output.PrintJSON(result)
		}
