// Generated by openapi-operator-gen v0.0.10-11-g5838925-dirty
...
var queryCmd = &cobra.Command{
	...
	Short: "Execute query CRDs for petstore",
	Long: `Execute query CRDs to fetch data from the petstore API.
...
Examples:
  ...
  kubectl petstore query petfindbytagsquery --tags=cute --wait
...
func runQuery(cmd *cobra.Command, args []string) error {
	...
	if queryKind == "" {
		return fmt.Errorf("unknown query type: %s\nRun 'kubectl petstore query --help' to see available types", queryType)
	}
	...
	if apierrors.IsAlreadyExists(err) {
		// Query already exists - delete and recreate to re-trigger execution
		if !queryQuiet {
			fmt.Printf("Query %s/%s already exists, re-executing...\n", plural, name)
		}

		deleteErr := k8sClient.Delete(ctx, plural, name)
		if deleteErr != nil && !apierrors.IsNotFound(deleteErr) {
			return fmt.Errorf("failed to delete existing query for re-execution: %w", deleteErr)
		}
		...
		for time.Now().Before(deleteDeadline) {
			_, getErr := k8sClient.Get(ctx, plural, name)
		...
		created, err = k8sClient.Create(ctx, plural, queryCR)
		if err != nil {
			return fmt.Errorf("failed to recreate query: %w", err)
		}
	} else if err != nil {
...
func isCommonFlag(name string) bool {
	if isTargetingFlag(name) || isClientGoFlag(name) {
		return true
	}
...
func buildQueryCR(kind, name string, params map[string]interface{}, isPeriodic bool) *unstructured.Unstructured {
	...
	if !isPeriodic {
		annotations["petstore.example.com/one-shot"] = "true"
...
var listQueriesCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Printf("  %-30s %s\n", "userlogoutquery", "UserLogoutQuery")
		fmt.Println()
		fmt.Println("Use 'kubectl petstore query <type> --help' for more information")
		return nil
	},
}
