...
func runAction(cmd *cobra.Command, args []string) error {
	...
	if apierrors.IsAlreadyExists(err) {
		// Action already exists - delete and recreate to re-trigger execution
		fmt.Printf("Action %s/%s already exists, re-executing...\n", plural, name)

		deleteErr := k8sClient.Delete(ctx, plural, name)
		if deleteErr != nil && !apierrors.IsNotFound(deleteErr) {
			return fmt.Errorf("failed to delete existing action for re-execution: %w", deleteErr)
		}
		...
		_, err = k8sClient.Create(ctx, plural, actionCR)
		if err != nil {
			return fmt.Errorf("failed to recreate action: %w", err)
		}
	} else if err != nil {
