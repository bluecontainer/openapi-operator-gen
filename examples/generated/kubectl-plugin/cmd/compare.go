...
// shortenEndpoint creates a short display name for an endpoint key.
// Endpoint keys are typically pod names (e.g. "petstore-0") or URLs as fallback.
func shortenEndpoint(endpoint string) string {
	// Handle URL fallback case (when pod name is not available)
