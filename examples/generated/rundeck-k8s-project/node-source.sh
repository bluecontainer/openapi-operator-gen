...
K8S_TOKEN=$(curl -sf \
  ...
  'http://localhost:4440/api/14/storage/keys/project/petstore-operator-k8s/k8s-token')

K8S_URL=$(curl -sf \
  -H 'X-Rundeck-Auth-Token: letmein99' \
  'http://localhost:4440/api/14/project/petstore-operator-k8s/config' \
...
kubectl --server="$K8S_URL" --token="$K8S_TOKEN" --insecure-skip-tls-verify \
  run "$POD" --image="$IMAGE" --restart=Never --rm -i --quiet --image-pull-policy=IfNotPresent \
  -n "$PLUGIN_NS" \
